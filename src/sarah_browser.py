...
class SarahBrowser:
    ...
    Usage:
        browser = SarahBrowser(headless=True, stream_port=8080)
        await browser.start()
    ...
    def __init__(self, headless: bool = False, stream_port: Optional[int] = None):
        ...
        # Initialize screen streamer if port provided
        if stream_port:
            self.streamer = PlaywrightScreenStreamer(port=stream_port, fps=2)
            self.logger.info(f"📺 Screen streamer initialized on port {stream_port}")
        else:
            self.streamer = None
    ...
    async def start(self):
        ...
        # Connect page to streamer if streaming is enabled
        if self.streamer:
            self.streamer.set_browser_page(self.page)
            self.logger.info("🎬 Browser connected to screen streamer")
